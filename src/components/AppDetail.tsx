...
export default function AppDetail(props: AppointmentDetailProps){
    const navigation = useNavigation();

    const handleMessagePress = () => {
        navigation.navigate('DirectMessages', {username: props.username, mostRecentMessage: props.mostRecentMessage})
    };

    return (
        <>
            <Avatar className='mx-auto mt-28 mb-20 w-32 h-32' source={{ uri: props.imageUrl }}></Avatar>
                ...
                <Box className='rounded-2xl bg-white border-[#4c8ab9] border-solid border-2'>
                    <Box className='flex-wrap flex-row mt-4 mb-1'>
                        <Button className='rounded-3xl bg-white w-14 h-14 flex-start'>
                            <Ionicons className='mx-auto' size={30} color='#4c8ab9' className='ml-2' name="paw"></Ionicons>
                        </Button>
                    ...
                    <Box className='flex-wrap flex-row my-1'>
                        ...
                        <Box className='flex-end'>
                            ...
                            <Text className='text-lg font-bold'>{props.dateDescription} {props.timeDescription}</Text>
                        </Box>
                    </Box>
                    <Divider my="2" _light={{bg: '#4c8ab9'}} _dark={{bg: "#4c8ab9"}} />
                    <Box className='flex-wrap flex-row my-1'>
